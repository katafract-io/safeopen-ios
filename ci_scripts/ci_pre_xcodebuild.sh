...
# train sane.
APP_ID="6761782681"   # SafeOpen
TRAIN=$(/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" \
  "$CI_PRIMARY_REPOSITORY_PATH/SafeOpen/Info.plist" 2>/dev/null \
...
if [ -z "$TRAIN" ] || [ "$TRAIN" = "\$(MARKETING_VERSION)" ]; then
  TRAIN=$(grep -m1 "MARKETING_VERSION = " "$CI_PRIMARY_REPOSITORY_PATH/SafeOpen.xcodeproj/project.pbxproj" \
